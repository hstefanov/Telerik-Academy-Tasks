...
function solveFirstTask(){
    ...
    if(!isNaN(a) && !isNaN(b) && a!== '' && b !== '') {
        if (a > b) {
            swap(a, b);
            document.getElementById('taskAnswer').innerHTML = '"a" and "b" swaped successfuly a =' + ' ' + a + ' b = ' + b ;
        } else{
            document.getElementById('taskAnswer').innerHTML = 'b is greater than a';
        }
...
function signOfNumber(n){
    return !isNaN(n) ? n ? n < 0 ? 'negative' : 'positive' : n === n ? 'equal' : NaN : NaN;
}

function solveTaskTwo(){
   var a = document.getElementById('first-number2').value,
    ...
    signs.push(signOfNumber(a));
    signs.push(signOfNumber(b));
    signs.push(signOfNumber(c));
    ...
    document.getElementById('taskTwoAsnwer').value = result;
