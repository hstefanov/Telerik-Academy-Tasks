namespace MathExpression
{
    ...
    public class ExpressionCalculator
    {
        ...
        {
            PutInvariantCulture();
            string input = "pow(2, 3.14) * (3 - (3 * sqrt(2) - 3.2) + 1.5*0.3) "; // -> ~ 21.22
            /*
            //user input
            string input = Console.ReadLine().Trim();
            */
            string trimmedInput = input.Replace(" ", string.Empty);
            var tokens = SeparateTokens(trimmedInput); // list            
            //reverse to polish notation
            var reversePolishNotation = ConvertToReversePolishNotation(tokens); // queue
